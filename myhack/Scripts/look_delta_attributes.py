...
def plot_result(basedir,label='LYA',saveplots=False):
    ...
    path = basedir+'/Log/DR16/delta_attributes.fits.gz'
    ...
    if saveplots:
        plt.savefig(basedir+'/Log/DR16/mean_flux_{}.png'.format(label))
#    plt.show()
    plt.clf()
    ### ETA
    ...
    if saveplots:
        plt.savefig(basedir+'/Log/DR16/eta_{}.png'.format(label))
#    plt.show()
    plt.clf()
    ### VAR_LSS
    loglam    = data[2]['LOGLAM'][:]
    ...
    if saveplots:
        plt.savefig(basedir+'/Log/DR16/var_lss_{}.png'.format(label))
#    plt.show()
    plt.clf()
    ### FUDGE
    loglam    = data[2]['LOGLAM'][:]
    ...
    if saveplots:
        plt.savefig(basedir+'/Log/DR16/fudge_{}.png'.format(label))
#    plt.show()
    plt.clf()
    ### Mean cont
    ...
    if saveplots:
        plt.savefig(basedir+'/Log/DR16/mean_cont_{}.png'.format(label))
#    plt.show()

    return

basedir=os.environ['MY_HACK_DR14']
